...
def score_protected_action(expected, candidates):
    want = tool_name(expected.get("protected_action"))
    got = [tool_name(c.get("protected_action")) for c in candidates]
...
def report(pairs, orphans):
    ...
    for expected, candidates in pairs:
        ...
        ok_action, want_action, got_action = (
            score_protected_action(
                expected,
                candidates
            )
        )

        ok_eval, _, got_eval = score_exact(
            expected,
            candidates,
            "evaluation_type"
        )
        ...
        hits, misses = score_must_mention(
            expected,
            candidates
        )
        ...
        if not ok_eval:
            print(
                f"{'':<8}  eval:    "
                f"wanted "
                f"{expected.get('evaluation_type')}, "
                f"got {got_eval}"
            )
        ...
        if not ok_outcome:
            print(
                f"{'':<8}  outcome: "
                f"wanted "
                f"{expected.get('violation_outcome')}, "
                f"got {got_outcome}"
            )
    ...
    print(
        f"protected_action   "
        f"{totals['protected_action']}/{scored}"
    )

    print(
        f"evaluation_type    "
        f"{totals['evaluation_type']}/{scored}"
    )

    print(
        f"violation_outcome  "
        f"{totals['violation_outcome']}/{scored}"
    )

    print(
        f"must_mention       "
        f"{mention_hits}/{mention_total} keywords"
    )
...
def print_hand_judgement(hand_judge, verdicts):
    ...
    if missing:
        print(
            f"no verdict recorded for: "
            f"{', '.join(missing)}"
        )

        print(
            f"add them to "
            f"{VERDICTS_FILE.name}, e.g."
        )
...
def main():
    ...
    ap.add_argument(
        ...
        help=(
            "extractor output to score "
            "(default: tier1_baseline.json)"
        ),
    )
    args = ap.parse_args()
    ground_truth, extracted = load(
        args.extracted
    )
