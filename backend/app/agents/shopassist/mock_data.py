...
CONFIDENTIAL_INFO: Dict[str, dict] = {c["id"]: c for c in _fixtures.get("confidential_facts", [])}
...
def get_policy_constant(key: str) -> Any:
    ...
    val = POLICY_CONSTANTS.get(key)
    if val is None and key == "manager_approval_threshold":
        return 500.0
    return val
