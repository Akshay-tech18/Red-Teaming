...
class GuardianAPI {
  ...
  normalizeAttack(a) {
    ...
    return {
      ...
      objective: a.objective || "Security test scenario",
      expected_violation: a.expected_violation || "Security policy violation",
        ...
        if (res.ok) {
          ...
          if (agentObj) {
            try {
              ...
              console.warn("Could not fetch versions:", vErr);
            }
            agentObj.tools = agentObj.tools || window.GUARDIAN_DATA.agent.tools;
            agentObj.policies = agentObj.policies || window.GUARDIAN_DATA.agent.policies;
            return agentObj;
          }
        }
      } catch (e) {
  ...
  async getAgentVersions(agentId = null) {
    if (this.isOnline) {
      try {
        ...
        if (!targetId) {
          ...
          if (agentsRes.ok) {
            ...
            if (agents && agents.length > 0) targetId = agents[0].id;
          }
        }
        if (targetId) {
          const res = await fetch(`${this.baseUrl}/agents/${targetId}/versions`);
  ...
  async getAttacks(constraintId = null) {
    ...
    const local = constraintId
      ? window.GUARDIAN_DATA.attacks.filter((a) => a.target_constraints.includes(constraintId))
    ...
    if (this.isOnline) {
      try {
        ...
        if (res.ok) {
          ...
          return this.normalizeAttack(attack);
        }
      } catch (e) {
        console.warn("API fallback to local attack detail:", e);
      }
    }
    const found = window.GUARDIAN_DATA.attacks.find((a) => a.id === attackId);
    return found ? this.normalizeAttack(found) : null;
  }
