        ...
        return (
          <React.Fragment key={step.id}>
            <button
              ...
              className={`flex-1 min-w-[130px] group relative flex items-center justify-center gap-2 px-3 py-2 rounded-md text-xs transition-all duration-200 border whitespace-nowrap ${
                isActive
            ...
            >
              {/* Number Badge (targeted hover glow) */}
              <span
                className={`font-mono text-[10.5px] font-bold px-1.5 py-0.5 rounded-sm shrink-0 transition-all duration-200 ${
                  isActive
            ...
            {idx < steps.length - 1 && (
              <ChevronRight className="w-3.5 h-3.5 text-slate-600/70 shrink-0 mx-0.5 select-none" />
            )}
          </React.Fragment>
        );
      })}
